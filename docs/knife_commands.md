...
* `--<box-type>-count` - This controls how many of each box type is created. In a GR deployment, this should be a comma separated list of how many of each box type is created in each site. In the traditional architecture, chef creates one each of a Bono, Sprout, Homer, Homestead and Ellis by default. In split state architecture, chef creates one each of a Bono, Sprout, Homer, Dime, Vellum and Ellis by default.
...
* `"num_gr_sites" => number of GR sites` - If this is set, the nodes will be created in multiple sites (to mimic a geographically redundant deployment). Deployments created using this method won't have a long latency between sites though.
...
In a GR, deployment, you can kill and recover sites to mimic site failure.

To kill a site, run:

    knife site kill -E <env> --site <site_index>

This kills all the nodes in site `site_index`, except for the Bono nodes.

To recover a killed site, run:

    knife site recover -E <env> --site <site_index>

This recovers all nodes in sites `site_index`.
